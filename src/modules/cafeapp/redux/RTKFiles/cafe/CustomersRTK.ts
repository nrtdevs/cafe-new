...
export const CustomerManagement = createApi({
  ...
  endpoints: (builder) => ({
    createOrUpdateCustomer: builder.mutation<ResponseType, RequestType>({
      query: (args) => ({
        jsonData: args?.jsonData,
        method: args?.jsonData?.id ? 'put' : 'post',
        path: ApiEndpoints.addCustomer + (args?.jsonData?.id ? '/' + args?.jsonData?.id : '')
      })
    }),
    viewCustomerById: builder.mutation<ResponseType, Number>({
      query: (args) => ({
        method: 'get',
        path: ApiEndpoints.viewCustomer + '/' + args
      })
    }),
    ...
    deleteCustomerById: builder.mutation<ResponseType, any>({
      query: ({ id }) => ({
        method: 'delete',
        path: ApiEndpoints.deleteCustomer + id
      })
    }),
    loadCustomers: builder.mutation<ResponseListType, RequestType>({
      query: (args) => ({
        jsonData: args?.jsonData,
