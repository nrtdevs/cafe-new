...
import { useWhatsAppLogMutation } from '@src/modules/cafeapp/redux/RTKFiles/cafe/OrderRTK'
...
const WhatsAppHistory = (props: any) => {
  ...
  const [loadWhatsApp, loadResp] = useWhatsAppLogMutation()
  ...
  const handleFilterData = (e: any) => {
    setState({
      filterData: {
        ...e,
        start_date: e.start_date ? formatDateValue(e?.start_date, 'YYYY-MM-DD') : '',
        end_date: e?.end_date ? formatDateValue(e?.end_date, 'YYYY-MM-DD') : ''
      },
      page: 1,
      search: '',
      per_page_record: 20
  ...
  const totalDiscount = state.selectedUser?.order_details
    .reduce((acc: any, d: any) => acc + (parseFloat(d.discount_amount) || 0), 0)
    .toFixed(2)
  ...
  const columns: TableColumn<orderResponseTypes>[] = [
    ...
    },
    {
      ...
      cell: (row) => <Fragment>{row?.order?.order_number}</Fragment>
    },
    ...
    },
    {
      ...
      sortable: false,
      minWidth: '160px',
      cell: (row) => <Fragment>{row?.receiver}</Fragment>
    },
    // {
    //   name: 'status',
    //   sortable: false,
    ...
    // },
    {
      ...
      cell: (row) => (
        <Fragment>
          <Fragment>{row?.wa_status}</Fragment>
        </Fragment>
      )
    },
  ...
  return (
    <Fragment>
      ...
      <Header route={props?.route} icon={<Menu size='25' />} title={FM('whats-app-history')}>
        <ButtonGroup color='dark'>
          <WhatsAppFilter handleFilterData={handleFilterData} />
          <LoadingButton
            tooltip={FM('reload')}
