...
export default function ItemImport<T>(props: T & dataType) {
  const {
    edit = null,
    noView = false,
    active = false,
  ...
  const [importItem, resultData] = useImportItemReportMutation()
  ...
  useEffect(() => {
    if (resultData?.isSuccess === true) {
      responseData(true)
      SuccessToast(FM('import-item-successfully'))
      closeModal()
    }
  }, [resultData])
  ...
  const sampleBooking = () => {
    ...
    setTimeout(() => {
      ...
      link.href = '/samples/item-demo-csv.csv' // public folder path
      link.download = 'item-demo-csv.csv'
