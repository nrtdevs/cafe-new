...
const WarehouseSubCategory = (props: any) => {
  ...
  useEffect(() => {
    if (!createCategoryResponse.isUninitialized) {
      if (createCategoryResponse.isSuccess) {
        ...
        SuccessToast(
          state?.editData?.id
            ? 'Updated Warehouse Sub Category Successfully'
            : 'Created Warehouse Sub Category Successfully'
        )
  ...
  }, [state.editData, state.editData?.name])
  ...
  return (
    <Fragment>
      <Header
        route={props?.route}
        icon={<ArrowLeft size='25' onClick={() => navigate(-1)} />}
        title='Warehouse Sub Categories'
      >
        ...
        <ButtonGroup color='dark'>
          <BsTooltip<ButtonProps>
            Tag={Button}
            title={state?.addCategory === true ? 'Cancel Sub Category' : 'Create Sub Category'}
            className='btn-icon'
            onClick={(e: any) => {
              setState({
                editData: null,
...
export default WarehouseSubCategory
