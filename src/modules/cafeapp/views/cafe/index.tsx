...
import { cafeResponseTypes } from '../../redux/RTKFiles/ResponseTypes'
import {
  useCreateOrUpdateCafeMutation,
  useDeletecafeByIdMutation,
  useLoadCafesMutation
} from '../../redux/RTKFiles/cafe-admin/cafeadminRTK'
...
const Cafe = (props: any) => {
  ...
  // can view cafe
  const canViewUser = Can(Permissions.cafeRead)
  // default states
  ...
  const [createCafe, createCafeResponse] = useCreateOrUpdateCafeMutation()
  // load users
  const [loadCafe, loadCafeResponse] = useLoadCafesMutation()
  const [ChildLogin, creatChildLogin] = useCafeChLoginMutation()

  // delete mutation

  const [cafeDelete, cafeRestDelete] = useDeletecafeByIdMutation()
  ...
  useEffect(() => {
    if (!createCafeResponse.isUninitialized) {
      if (createCafeResponse.isSuccess) {
        ...
        SuccessToast(
          state?.editData?.id ? (
            <>{FM('updated-cafe-successfully')}</>
          ) : (
            <> {FM('created-cafe-successfully')}</>
          )
        )
  ...
  //cafe child
  useEffect(() => {
    if (!creatChildLogin.isUninitialized) {
      if (creatChildLogin.isSuccess) {
        // closeAddModal()nRJ&1234
        // loadUserList()

        const data = creatChildLogin.data
      ...
      } else if (creatChildLogin.isError) {
        // handle error
        const errors: any = creatChildLogin.error
        log(errors)
  ...
  useEffect(() => {
    if (cafeRestDelete?.isLoading === false) {
      if (cafeRestDelete?.isSuccess) {
        emitAlertStatus('success', null, cafeRestDelete?.originalArgs?.eventId)
      } else if (cafeRestDelete?.error) {
        emitAlertStatus('failed', null, cafeRestDelete?.originalArgs?.eventId)
      }
    }
  }, [cafeRestDelete])
  ...
  const renderCreateModal = () => {
    return (
      <CenteredModal
        ...
        title={state.enableEdit ? <>{FM('update-cafe')}</> : <>{FM('create-cafe')}</>}
      ...
      >
        <div className='p-2'>
          <Row>
            ...
            <Col md='6'>
              <Label className='text-uppercase mb-25'>{'cafe status'}</Label>
              <p className=''>{state.selectedUser?.status === 1 ? 'Active' : 'InActive'}</p>
            </Col>

            <Col md='6'>
              <Label className='text-uppercase mb-25'>{FM('no-of-subcafe')}</Label>
              <p className=''>{state.selectedUser?.no_of_subcafe ?? 'N/A'}</p>
            </Col>
            <Col md='6'>
              <Label className='text-uppercase mb-25'>{'subscription status'}</Label>
              <p className=''>
                {state.selectedUser?.subscription_status === 1 ? (
            ...
            <Col md='6'>
              <Label className='text-uppercase mb-25'>{'website'}</Label>
              <p className=''>{state.selectedUser?.website ?? 'N/A'}</p>
            </Col>
            <Col md='6'>
              <Label className='text-uppercase mb-25'>{'contact person name'}</Label>
              <p className='text-capitalize'>{state.selectedUser?.contact_person_name ?? 'N/A'}</p>
            </Col>
            <Col md='6'>
              <Label className='text-uppercase mb-25'>{'contact person email'}</Label>
              <p className=''>{state.selectedUser?.contact_person_email ?? 'N/A'}</p>
            </Col>
            <Col md='6'>
              <Label className='text-uppercase mb-25'>{'Contact person phone'}</Label>
  ...
  const columns: TableColumn<cafeResponseTypes>[] = [
    ...
    {
      name: FM('no-of-subcafe'),
      sortable: false,
      id: 'no_of_subcafe',
      cell: (row) => <Fragment>{row?.no_of_subcafe}</Fragment>
    },
    {
      name: FM('status'),
      sortable: false,
  ...
  return (
    <Fragment>
      ...
      <Header route={props?.route} icon={<List size='25' />} title='Cafe List'>
        <ButtonGroup color='dark'>
          <CafeFilter handleFilterData={handleFilterData} />
