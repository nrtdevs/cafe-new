...
import { ProjectsParam, UserData } from '@src/utility/types/typeAuthApi'
...
const TaskList = (props: any) => {
    ...
    // load users
    const [loadUsers, loadUserResponse] = useProjectListMutation()
    // delete mutation
    ...
    const renderViewModal = () => {
        return (
            <CenteredModal
                open={modalView}
                title={state.selectedUser?.project_name}
                done='edit'
                hideClose
                disableFooter
            ...
            <Col md='8'>
              ...
              <p className='text-muted small mb-0'>{FM('edit-description')}</p>
                    ...
                    <Row>
                        <Col md='6'>
                            <Label className='text-uppercase mb-25'>Project Name</Label>
                            <p className='text-dark fw-bold text-capitalize'>
                                {state.selectedUser?.project_name}
                        ...
                        <Col md='6'>
                            <Label className='text-uppercase mb-25'>Project Code</Label>
                            <p className='text-dark fw-bold text-capitalize'>
                                {state.selectedUser?.project_code ?? 'N/A'}
                            </p>
                        </Col>
                        <Col md='6'>
    ...
    // table columns
    const columns: TableColumn<ProjectsParam>[] = [
        {
            name: 'Project ID',
            sortable: true,
            id: 'name',
        ...
        },
        {
            ...
            sortable: true,
            id: 'email',
            cell: (row) => <Fragment>{row?.project_code}</Fragment>
        },
        {
            name: 'Start Date',
            sortable: true,
            id: 'mobile_number',
            cell: (row) => <Fragment>{formatDate(row?.start_date)}</Fragment>
        },
        {
            name: 'End Date',
            sortable: true,
            id: 'task',
            cell: (row) => <Fragment>{row?.end_date}</Fragment>
        },

        {
            ...
            cell: (row) => (
                <Fragment>
                    <span className={'text-success'}>{row?.status_code}</span>
                </Fragment>
            )
        }
        // {
    ...
    return (
        <Fragment>
            {renderViewModal()}
            <Header route={props?.route} icon={<List size='25' />} title='Projects'>
